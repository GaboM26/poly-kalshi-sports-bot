    ...
    <div className="p-3 space-y-3">
      ...
      <div className="bg-[--bg-tertiary] rounded p-2.5">
        <div className="text-[10px] text-[--text-muted] mb-2 flex items-center gap-1">
          <span>🔗</span> API 延迟（下单接口）
        </div>
        <div className="grid grid-cols-2 gap-2">
          <div className={`rounded p-2 ${getLatencyBg(metrics.api_latency.kalshi_ms)}`}>
            <div className="text-[10px] text-[--text-muted] mb-1">Kalshi</div>
            <div className={`text-lg font-bold font-mono ${getLatencyColor(metrics.api_latency.kalshi_ms)}`}>
              {formatLatency(metrics.api_latency.kalshi_ms)}
            </div>
          </div>
          <div className={`rounded p-2 ${getLatencyBg(metrics.api_latency.polymarket_ms)}`}>
            <div className="text-[10px] text-[--text-muted] mb-1">Polymarket</div>
            <div className={`text-lg font-bold font-mono ${getLatencyColor(metrics.api_latency.polymarket_ms)}`}>
              {formatLatency(metrics.api_latency.polymarket_ms)}
            </div>
          </div>
        </div>
      </div>
      ...
      {/* 延迟说明 */}
      <div className="text-[9px] text-[--text-muted] space-y-1 px-1">
